...
export const TodolistsReducer = (state:Array<TodolistDomainType>=initialState1,action:AppActionsType):Array<TodolistDomainType> => {
  switch (action.type) {
      ...
      case "ADD-TODOLIST":
          let newState: TodolistDomainType = {id: action.todolistId, addedDate: '5478488923',order: 0,title: action.title, filter: 'All'}
...
export const changeTitleTodolistAC = (title:string,id:string) =>({ type: "CHANGE-TODOLIST-TITLE", id: id, title: title} as const)
