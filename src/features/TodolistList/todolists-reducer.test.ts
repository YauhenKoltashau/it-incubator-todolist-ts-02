...
const {fetchTodolistsThunk, removeTodolistThunk, changeFilterTodolist, changeTodolistEntityStatusAC} = todolistsActions
beforeEach(()=>{
    todolistId_1 = v1()
    todolistId_2 = v1()
...
test('correct todolist should be reducer',()=>{
    const startStateWithDomain = TodolistsReducer([],fetchTodolistsThunk.fulfilled({todolists: startState},'requestId'))
    expect(startStateWithDomain.length).toBe(2)
    expect(startStateWithDomain[0].filter).toBeDefined()
    const endState = TodolistsReducer(startStateWithDomain, removeTodolistThunk.fulfilled({id:todolistId_2},'requestId', {todolistId:todolistId_2} ))
...
test('correct filter of todolist should be changed', () => {
    let newFilter: types.FilterValuesType = "Completed";
    const action = changeFilterTodolist({
        id: todolistId_2,
        filter: newFilter
    })
    const startStateWithFilter = TodolistsReducer([],fetchTodolistsThunk.fulfilled({todolists:startState}, 'requestId'))
